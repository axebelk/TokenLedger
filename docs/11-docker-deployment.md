...
x-app-env: &app-env
  DATABASE_URL: postgresql://tokenledger:${POSTGRES_PASSWORD}@postgres:5432/tokenledger
  REDIS_URL: redis://redis:6379
  TOKENLEDGER_MASTER_KEY: ${TOKENLEDGER_MASTER_KEY}      # openssl rand -base64 32
...
services:
  postgres:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U tokenledger"]
      interval: 5s
      timeout: 3s
      retries: 12
  ...
  migrate:                       # one-shot: schema migrations + pricing seed
    image: ghcr.io/tokenledger/api:${TOKENLEDGER_VERSION:-latest}
  ...
  gateway:
    image: ghcr.io/tokenledger/gateway:${TOKENLEDGER_VERSION:-latest}
    environment:
      <<: *app-env
      PORT: "4100"
  ...
  worker:
    image: ghcr.io/tokenledger/worker:${TOKENLEDGER_VERSION:-latest}
    environment:
      <<: *app-env
      SMTP_URL: ${SMTP_URL:-}
  ...
  web:
    image: ghcr.io/tokenledger/web:${TOKENLEDGER_VERSION:-latest}   # nginx static
    restart: unless-stopped
...
```
Set `DOMAIN=tokenledger.example.com` in `.env` → Caddy provisions Let's Encrypt automatically.
...
TOKENLEDGER_MASTER_KEY=            # openssl rand -base64 32  (encrypts provider keys — losing it orphans them)
JWT_SECRET=                       # openssl rand -base64 48

# ── Instance ────────────────────────────────────────────────────
PUBLIC_BASE_URL=http://localhost:8080
TOKENLEDGER_VERSION=latest
GATEWAY_REPLICAS=1
...
COPY ee ./ee
RUN pnpm install --frozen-lockfile && pnpm turbo build --filter=@tokenledger/gateway
RUN pnpm deploy --filter=@tokenledger/gateway --prod /out
...
| Metrics | every service exposes `/metrics`; optional `--profile observability` adds Prometheus + Grafana with bundled TokenLedger dashboards |
